...
export const subscriptionService = {
  ...
  /**
 * Subscribe to a user/channel directly by userId
 * @param {string} userId - ID of the user you want to subscribe to
 * @returns {Promise} - API response confirming subscription
 */
subscribeUser: async (userId) => {
  try {
    const response = await api.post(`/subscription/subscribe/user/${userId}`);
    return response.data.data;
  } catch (error) {
...
/**
 * Unsubscribe from a user/channel directly by userId
 * @param {string} userId - ID of the user you want to unsubscribe from
 * @returns {Promise} - API response confirming unsubscription
 */
unsubscribeUser: async (userId) => {
  try {
    const response = await api.post(`/subscription/unsubscribe/user/${userId}`);
    return response.data.data;
  } catch (error) {
